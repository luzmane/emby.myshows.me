...
using EmbyMyShowsMe.ScheduledTasks.Model;
...
namespace EmbyMyShowsMe.ScheduledTasks
{
    /// <summary>
    /// Task to update the MyShowsMe plugin
    /// </summary>
    public class UpdateMyShowsMePluginTask : BaseTask, IScheduledTask, IConfigurableScheduledTask
    {
        private static readonly Regex Version = new Regex(@".*(?<version>\d+\.\d+\.\d+).*", RegexOptions.Compiled);

        private const string DllName = "EmbyMyShowsMe.dll";
        private const string TaskKey = "MyShowsMeNewVersion";
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UpdateMyShowsMePluginTask"/> class.
        /// </summary>
        ...
        public UpdateMyShowsMePluginTask(
        ...
        {
            Version version = typeof(UpdateMyShowsMePluginTask).Assembly.GetName().Version ?? throw new Exception("Unable to get dll version");
        ...
        private async Task<GitHubLatestReleaseResponse> GetGitHubLatestReleaseAsync(CancellationToken cancellationToken)
        {
            var options = new HttpRequestOptions
            {
                ...
                Url = "https://api.github.com/repos/luzmane/emby.myshows.me/releases/latest",
                LogResponse = true,
